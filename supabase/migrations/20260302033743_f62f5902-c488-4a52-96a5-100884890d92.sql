CREATE OR REPLACE FUNCTION public.search_filaments_paginated(p_search text DEFAULT NULL::text, p_materials text[] DEFAULT NULL::text[], p_brands text[] DEFAULT NULL::text[], p_sort text DEFAULT 'scoring-desc'::text, p_page_size integer DEFAULT 48, p_offset integer DEFAULT 0, p_region text DEFAULT 'US'::text, p_high_speed boolean DEFAULT false, p_brass_only boolean DEFAULT false, p_ams_only boolean DEFAULT false, p_matte boolean DEFAULT false, p_silk boolean DEFAULT false, p_metallic boolean DEFAULT false, p_sparkle boolean DEFAULT false, p_translucent_filter boolean DEFAULT false, p_glow boolean DEFAULT false, p_carbon_fiber boolean DEFAULT false, p_glass_fiber boolean DEFAULT false, p_wood_filled boolean DEFAULT false, p_large_spools boolean DEFAULT false, p_has_td boolean DEFAULT false, p_price_min numeric DEFAULT NULL::numeric, p_price_max numeric DEFAULT NULL::numeric, p_color_families text[] DEFAULT NULL::text[], p_affiliate_boost boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  sorted AS (
    ...
    FROM grouped g
    ORDER BY
      -- Affiliate boost: only when default sort AND boost requested
