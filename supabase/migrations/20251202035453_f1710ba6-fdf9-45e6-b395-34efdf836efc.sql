-- Function to prevent duplicate printer names with "3D Printer" suffix
...
-- Create trigger on printers table
DROP TRIGGER IF EXISTS check_printer_name_duplicates ON printers;
CREATE TRIGGER check_printer_name_duplicates
  BEFORE INSERT OR UPDATE OF model_name, brand_id
  ON printers
  FOR EACH ROW
  EXECUTE FUNCTION prevent_printer_name_duplicates();