...
CREATE OR REPLACE FUNCTION public.search_filaments_paginated(
  ...
  p_matte boolean DEFAULT false,
  p_silk boolean DEFAULT false,
  p_metallic boolean DEFAULT false,
  p_sparkle boolean DEFAULT false,
  p_translucent_filter boolean DEFAULT false,
  p_glow boolean DEFAULT false,
  p_carbon_fiber boolean DEFAULT false,
  p_glass_fiber boolean DEFAULT false,
  p_wood_filled boolean DEFAULT false,
  p_large_spools boolean DEFAULT false,
  p_has_td boolean DEFAULT false,
  p_price_min numeric DEFAULT NULL,
  p_price_max numeric DEFAULT NULL,
  p_color_families text[] DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  filtered AS (
    ...
    WHERE f.material IS NOT NULL
      ...
      -- Regional filtering REMOVED: all products should be visible regardless of region
      -- Regional URLs only affect which store link is shown, not product visibility
  ),
  grouped AS (
    SELECT 
