...
function generateVoxelPLAFixPrompt(
  brand: string,
  checks: CheckResult[],
  totalProducts: number,
  aiAnalysis?: AIWebsiteAnalysis | null
...
// ============================================================================
// ZIRO AI FIX PROMPT GENERATOR (Enhanced with issue-specific fix sections)
// ============================================================================

function generateZiroFixPromptEnhanced(
  checks: CheckResult[],
  totalProducts: number,
  aiAnalysis?: AIWebsiteAnalysis | null
): string {
...
**Expected:** 30 cards (defined in EXPECTED_CARD_COUNTS)
**Found:** ${cardInfo.count} cards
...
**File:** \`supabase/functions/run-post-sync-check/index.ts\` (local override ~line 10567)
\`\`\`typescript
...
**ADD TO skipUrlCheckBrands (around line 11165):**
\`\`\`typescript
const skipUrlCheckBrands = ['3dfuel', 'polymaker', 'ziro', /* ... */];
\`\`\`
`;
    priorityFixes.push('4. **Fix URL Consistency** - Add Ziro to `skipUrlCheckBrands` (RC3)');
  }
...
**ADD TO isSingleColorProduct check (around line 11954):**
\`\`\`typescript
// Ziro single-variant product lines (effect-based products)
lineId.includes('ziro__pla__rainbow-glow') ||
lineId.includes('ziro__pla__stone-blue-white') ||
lineId.includes('ziro__pla__marble') ||
lineId.includes('ziro__pla__straw-fiber') ||
\`\`\`
...
\`\`\`
`;
    priorityFixes.push('5. **Fix Variant Count** - Add single-variant lines to whitelist (RC4)');
  }
...
`;
  ...
  if (aiAnalysis) {
    ...
    const swatchArch = analysis.swatchArchitecture || 'cross-product (image-alt based)';
    const extractPattern = analysis.extractionPattern || 'Color extracted from product title and URL path';
    const rootCause = analysis.rootCauseAnalysis ? `**Root Cause Analysis:**\n${analysis.rootCauseAnalysis}` : '';
    const missingColors = Array.isArray(analysis.missingColorMappings) ? analysis.missingColorMappings as string[] : [];
    const missingColorsSection = missingColors.length > 0 
      ? `\n**Missing Colors to Add:**\n\`\`\`typescript\n// Add to ZIRO_COLOR_MAPPING in ziro-defaults.ts:\n${missingColors.map((c: string) => "'" + c.toLowerCase() + "': '#000000', // TODO: Add correct hex").join('\n')}\n\`\`\`\n`
      : '';
...
**Swatch Architecture:** ${swatchArch}

**Extraction Pattern:** ${extractPattern}
...
function generateAIFixPrompt(
  ...
  // Use brand-specific prompt generator for Ziro (enhanced modular version)
  if (brandSlug === 'ziro') {
    return generateZiroFixPromptEnhanced(checks, totalProducts, aiAnalysis);
  }
