 ...
 */
// NOTE: generateSunluFixPrompt and generateSpectrumFilamentsFixPrompt
// have been moved to shared modules in _shared/post-sync-check/brand-prompts/
// They are now imported via getBrandFixPrompt from the index.ts router

// NOTE: generateRecreusFixPrompt, generateVoxelPLAFixPrompt, and generateZiroFixPromptEnhanced
// have been moved to shared modules in _shared/post-sync-check/brand-prompts/
// They are now imported via getBrandFixPrompt from the index.ts router
...
function generateAIFixPrompt(
  ...
  // TreeD Filaments now uses generic prompt (modularized generator removed)
  ...
  // Use shared module for modularized brands (ziro, recreus, voxelpla)
  if (MODULARIZED_BRANDS.includes(brandSlug as typeof MODULARIZED_BRANDS[number])) {
