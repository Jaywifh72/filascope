...
const STATIC_PG: Record<string,SP> = {
  ...
  "/colors":{t:"Filament Color Finder — Browse 22,000+ Colors",d:"Find the exact filament color you need. Search 22,000+ 3D printer filaments by hex code, color family, or shade from 49+ brands.",c:"/colors",h:"3D Filament Color Finder",b:"Search for 3D printer filaments by color."},
  "/color-finder":{t:"Filament Color Finder — Browse 22,000+ Colors",d:"Find the exact filament color you need. Search 22,000+ 3D printer filaments by hex code, color family, or shade from 49+ brands.",c:"/colors",h:"3D Filament Color Finder",b:"Search for 3D printer filaments by color."},
  ...
  "/hueforge-tools":{t:"HueForge Tools — TD Lookup, Color Matcher & More | FilaScope",d:"Free HueForge tools: TD value lookup, color matcher, palette builder, layer preview. Everything you need for HueForge lithophane printing.",c:"/hueforge-tools",h:"HueForge Tools",b:"Free tools for HueForge lithophane printing."},
  "/matrix":{t:"Filament Compatibility Matrix — Printer × Material | FilaScope",d:"Check which filament materials work with your 3D printer. Interactive compatibility matrix.",c:"/matrix",h:"Filament Compatibility Matrix",b:"See which filaments are compatible with your 3D printer."},
};
...
const EXCLUDED_STATIC_PATHS = new Set([
  "/robots.txt",
  "/llms.txt",
  "/llms-full.txt",
  "/manifest.json",
...
function isStaticFilePath(p: string): boolean {
  return (
    STATIC_FILE_RE.test(p) ||
    p.startsWith("/assets/") ||
    p.startsWith("/public/") ||
    p.startsWith("/sitemap-") ||
    EXCLUDED_STATIC_PATHS.has(p)
  );
}
