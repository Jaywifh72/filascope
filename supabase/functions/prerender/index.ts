...
const EXCLUDED_STATIC_PATHS = new Set(["/robots.txt", "/llms.txt", "/llms-full.txt", "/sitemap.xml", "/manifest.json", "/sw.js"]);
function isStaticFilePath(p: string): boolean {
  // Allow sitemap sub-files through (they are generated by this function)
  if (p.startsWith("/sitemap-")) return false;
