...
function buildHtml(d: PageData): string {
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/><title>${esc(d.title)}</title><meta name="description" content="${esc(d.description)}"/><link rel="canonical" href="${esc(cu)}"/>${pl}<meta property="og:title" content="${esc(d.title)}"/><meta property="og:description" content="${esc(d.description)}"/><meta property="og:url" content="${esc(cu)}"/><meta property="og:type" content="${d.ogType}"/><meta property="og:image" content="${esc(oi)}"/><meta property="og:site_name" content="FilaScope"/><meta name="twitter:card" content="summary_large_image"/><meta name="twitter:title" content="${esc(d.title)}"/><meta name="twitter:description" content="${esc(d.description)}"/><meta name="twitter:image" content="${esc(oi)}"/>${d.modifiedTime?`<meta property="article:modified_time" content="${esc(d.modifiedTime)}"/>`:``}${jld}</head><body><div id="root"><header><nav aria-label="Breadcrumb">${bc}</nav></header><main><h1>${esc(d.h1)}</h1><p>${esc(d.bodyText)}</p>${bl}</main><noscript><p>FilaScope requires JavaScript.</p></noscript></div></body></html>`;
}
