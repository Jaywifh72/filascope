...
const EXCLUDED_STATIC_PATHS = new Set(["/robots.txt", "/llms.txt", "/llms-full.txt", "/manifest.json", "/sw.js"]);
function isStaticFilePath(p: string): boolean {
  // Allow all sitemap files through (they are generated by this function)
  if (p === "/sitemap.xml" || p.startsWith("/sitemap-")) return false;
  return STATIC_FILE_RE.test(p) || p.startsWith("/assets/") || EXCLUDED_STATIC_PATHS.has(p);
}
