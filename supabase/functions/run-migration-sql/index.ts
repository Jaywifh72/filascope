...
CREATE INDEX IF NOT EXISTS brand_sync_logs_brand_started
  ON brand_sync_logs(brand_slug, started_at DESC);
CREATE INDEX IF NOT EXISTS brand_sync_logs_started
  ON brand_sync_logs(started_at DESC);
...
CREATE INDEX IF NOT EXISTS sync_heal_log_run_at ON sync_heal_log(run_at DESC);
CREATE INDEX IF NOT EXISTS sync_heal_log_brand ON sync_heal_log(brand_slug, run_at DESC);
...
serve(async (req) => {
  ...
  try {
    ...
    try {
      // Split by semicolon+newline, filter empty, execute
      ...
      const healLogCheck = await client.queryObject(`
        SELECT 1 FROM information_schema.tables WHERE table_name = 'sync_heal_log';
      `);

      return new Response(JSON.stringify({
        success: true,
        brand_sync_logs_columns_added: colCheck.rows.length,
        columns: colCheck.rows,
        sync_heal_log_created: healLogCheck.rows.length > 0,
        statements_executed: stmts.length
