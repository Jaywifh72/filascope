    ...
    <Collapsible open={isOpen} onOpenChange={setIsOpen}>
      ...
      <CollapsibleContent>
        <div className="border border-t-0 rounded-b-lg p-4 space-y-4 bg-card/50">
          ...
          {/* Expert Fix Prompt Section */}
          {summary.lovablePrompt && (
            <Collapsible open={promptOpen} onOpenChange={setPromptOpen}>
              <CollapsibleTrigger asChild>
                <div className="flex items-center justify-between p-3 bg-primary/5 border border-primary/20 rounded-lg cursor-pointer hover:bg-primary/10 transition-colors">
                  <div className="flex items-center gap-2">
                    <Wrench className="w-4 h-4 text-primary" />
                    <span className="text-sm font-medium text-primary">Expert Fix Prompt for Lovable</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button 
                      variant="outline" 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleCopyPrompt(summary.lovablePrompt!);
                      }}
                    ...
                    {promptOpen ? <ChevronDown className="w-4 h-4" /> : <ChevronRight className="w-4 h-4" />}
                  </div>
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
                <div className="mt-2 p-4 bg-muted/50 border border-border rounded-lg">
                  <ScrollArea className="max-h-[400px]">
                    <pre className="text-xs text-muted-foreground whitespace-pre-wrap font-mono leading-relaxed pr-4">
                      {summary.lovablePrompt}
                    </pre>
                  </ScrollArea>
                  <p className="text-xs text-muted-foreground/60 mt-3 flex items-center gap-1">
                    <Sparkles className="w-3 h-3" />
                    This prompt was generated by AI to help fix the detected issues
                  </p>
                </div>
              </CollapsibleContent>
            </Collapsible>
