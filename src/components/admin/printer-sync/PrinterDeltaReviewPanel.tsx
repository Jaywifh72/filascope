...
import { NewPrintersTable } from './NewPrintersTable';
import { PrinterPriceChangesTable } from './PrinterPriceChangesTable';
import { PrinterPreviewDialog } from './PrinterPreviewDialog';
import type { PrinterSyncItem } from '@/types/printer-sync';
import type { DeltaStats } from '@/hooks/useCatalogSync';
...
interface Props {
  ...
  importing: boolean;
  onUpdateItem?: (itemId: string, overrides: Record<string, unknown>) => void;
  onSkipItem?: (itemId: string) => void;
}

export function PrinterDeltaReviewPanel({ items, stats, onStartImport, importing, onUpdateItem, onSkipItem }: Props) {
  const [selectedItems, setSelectedItems] = useState<Set<string>>(new Set());
  const [previewItem, setPreviewItem] = useState<PrinterSyncItem | null>(null);
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <StatCard
            ...
            tooltip="Existing printers from this brand already in the FilaScope database"
          />
          <StatCard
            icon={<Sparkles className="w-4 h-4 text-blue-500" />}
            label="New Available"
            value={stats.newCount}
            tooltip="Printers found in the store that are not yet in the database"
            highlight="blue"
          />
          <StatCard
            ...
            tooltip="Existing printers where the store price has changed since last sync"
            highlight="amber"
          />
          <StatCard
            ...
            <CardContent className="pt-4">
              <TabsContent value="new" className="mt-0">
                {newItems.length === 0 ? (
                  <p className="text-sm text-muted-foreground py-8 text-center">
                    No new printers found — the database is up to date with this brand's store.
                  </p>
                ) : (
                  <NewPrintersTable
                    items={newItems}
              ...
              <TabsContent value="changed" className="mt-0">
                {changedItems.length === 0 ? (
                  ...
                  <PrinterPriceChangesTable items={changedItems} onPreview={setPreviewItem} />
                )}
              </TabsContent>
                ...
                {matchedItems.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    <p className="text-sm text-muted-foreground mb-3">
                      These {matchedItems.length} printers are already in the database and match the store data.
                    </p>
                    <div className="grid gap-1 max-h-[400px] overflow-y-auto">
                      {matchedItems.map(item => (
                        <div key={item.id} className="flex items-center gap-2 py-1 px-2 text-sm rounded hover:bg-muted/50">
                          ...
                          <span className="text-xs text-muted-foreground ml-auto">{item.printer_technology}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <div className="sticky bottom-0 z-10 bg-background/95 backdrop-blur border-t border-border p-4 -mx-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <span className="text-sm font-medium">
                {selectedItems.size} printer{selectedItems.size !== 1 ? 's' : ''} selected
              </span>
              {changedItems.length > 0 && (
        ...
        {previewItem && (
          <PrinterPreviewDialog
            ...
            onSave={(itemId, overrides) => {
              onUpdateItem?.(itemId, overrides);
              setPreviewItem(null);
            }}
            onSkip={(itemId) => {
              onSkipItem?.(itemId);
...
function StatCard({
  icon, label, value, tooltip, highlight,
}: {
  icon: React.ReactNode; label: string; value: number; tooltip: string;
