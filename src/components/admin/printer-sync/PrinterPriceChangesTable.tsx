...
export function PrinterPriceChangesTable({ items, onPreview }: Props) {
  type DiffRow = {
        ...
        {rows.length === 0 ? (
          ...
          return (
            <TableRow key={`${row.item.id}-${row.field}-${i}`}>
              ...
              <TableCell>
                {row.item.printer_technology ? (
                  <Badge variant="outline" className="text-[10px]">{row.item.printer_technology}</Badge>
                ) : '—'}
              </TableCell>
