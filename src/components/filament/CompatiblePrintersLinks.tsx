  ...
  const { data: printers } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("printers")
        .select("id, model_name, display_name, brand:printer_brand_id(brand)")
    ...
    <section aria-label="Compatible 3D printers" className="mt-4 mb-4">
      ...
      <nav className="flex flex-wrap gap-2">
        {printers.map((printer) => {
          const brandName = (printer.brand as any)?.brand || '';
          const modelName = printer.display_name || printer.model_name;
          // Prepend brand if not already in model name
          const fullName = brandName && !modelName.toLowerCase().startsWith(brandName.toLowerCase())
            ? `${brandName} ${modelName}`
            : modelName;
          return (
            <a
              ...
              title={`${fullName} — compatible with ${material || 'this filament'}`}
        ...
        >
          Browse all compatible printers →
        </a>
      </nav>
    </section>
  );
