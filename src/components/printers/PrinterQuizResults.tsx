import { X, Check, Info, RotateCcw, ArrowRight, ExternalLink } from 'lucide-react';
...
const PrinterQuizResults = ({ results, onClose, onRetake, onAddToCompare }: PrinterQuizResultsProps) => {
  ...
  const handleViewDetails = (printerId: string) => {
    onClose();
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-[900px] my-8 bg-card border border-border rounded-2xl shadow-2xl animate-in slide-in-from-bottom-4 duration-300"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-start px-6 md:px-8 py-6 border-b border-border">
        ...
        <div className="px-6 md:px-8 py-6 space-y-6">
          {topPrinters.map((scored, index) => (
            <RecommendationCard
              ...
              onViewDetails={() => handleViewDetails(scored.printer.id)}
              onAddToCompare={onAddToCompare ? () => onAddToCompare(scored.printer.id) : undefined}
            />
          ))}
        ...
        {/* Footer */}
        <div className="flex flex-wrap gap-3 justify-center px-6 md:px-8 py-6 border-t border-border">
          <Button
            variant="ghost"
            onClick={onRetake}
...
const RecommendationCard = ({ 
  ...
  return (
    ...
    `}>
      {/* Rank Badge */}
      <div className={`
        inline-flex items-center justify-between w-full mb-5 px-4 py-2.5 rounded-lg
        ${isTopPick 
      ...
      <div className="flex gap-5 mb-6">
        <div className="w-28 h-28 rounded-xl bg-muted/50 border border-border p-3 flex-shrink-0 flex items-center justify-center">
          <img 
