...
export const BRAND_SYNC_DOCS: Record<string, BrandSyncDoc> = {
  ...
  'amolen': {
    ...
    specialBehaviors: [
      ...
      { name: 'Granular Product Line IDs', description: 'Separates S-Series, Dual, Triple, Rainbow, Galaxy, Matte, Crystal variants.' },
      { name: 'PEBA Material Detection', description: 'PEBA 90A flexible material separated from PLA products.' },
      { name: 'Carbon Fiber Separation', description: 'PLA-CF and PETG-CF grouped distinctly from base materials.' },
      { name: 'High-Speed Line Detection', description: 'HS-PLA products flagged as high_speed_capable.' },
      { name: 'Prioritized Hex Lookup', description: 'Partial matching ensures distinct colors resolve correctly (e.g., "Basic Cold White" vs "Basic White").' },
    ],
    notes: [
      'Shopify API-based pipeline',
      'Uses actual Shopify product titles (not constructed)',
      '20+ product lines including engineering materials',
    ],
  },
